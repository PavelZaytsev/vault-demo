...
node_2_api=${NODE_2_API}
node_2_cluster=${NODE_2_CLUSTER}

node_3_api=${NODE_3_API}
node_3_cluster=${NODE_3_CLUSTER}

node_4_api=${NODE_4_API}
node_4_cluster=${NODE_4_CLUSTER}
...
function vault_to_network_address {
  ...
  case "$vault_node_name" in
    vault_1)
      ...
      ;;
    vault_2)
      echo "http://${node_2_api}"
      ;;
    vault_3)
      echo "http://${node_3_api}"
      ;;
    vault_4)
      echo "http://${node_4_api}"
      ;;
  esac
}
...
EOF
  ...
  listener "tcp" {
    address = "$node_2_api"
    cluster_address = "$node_2_cluster"
    tls_disable = true
  }
  seal "transit" {
    address            = "http://${node_1}"
  ...
  disable_mlock = true
  cluster_addr = "http://${node_2_cluster}"
EOF
  ...
  listener "tcp" {
    address = "$node_3_api"
    cluster_address = "$node_3_cluster"
    tls_disable = true
  }
  seal "transit" {
    address            = "http://${node_1}"
  ...
  disable_mlock = true
  cluster_addr = "http://${node_3_cluster}"
EOF
  ...
  listener "tcp" {
    address = "$node_4_api"
    cluster_address = "$node_4_cluster"
    tls_disable = true
  }
  seal "transit" {
    address            = "http://${node_1}"
  ...
  disable_mlock = true
  cluster_addr = "http://${node_4_cluster}"
EOF
  printf "\n"
}
...
function create {
  case "$1" in
    ...
    *)
      ...
      "Usage: $script_name create [network|config] $2" \
      ""
      ;;
  esac
}
